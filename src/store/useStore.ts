...
export type WallPhoto = { src: string; title: string; ratio?: string; media_type?: string; cover_url?: string; videoRatio?: string };
...
export const useStore = create<AppState>((set, get) => ({
  ...
  loadWallPhotos: async () => {
    try {
      ...
      set({
        wallPhotos: photos.map(p => ({ src: p.src, title: p.name, ratio: p.ratio, media_type: p.media_type, cover_url: p.cover_url, videoRatio: p.video_ratio })),
        wallLoading: false,
      });
    } catch {
    ...
    const channel = db.supabase
      ...
      .on(
        ...
        async () => {
          ...
          const photos = await db.fetchPhotos();
          const wallPhotos = photos.map((p: SupabasePhoto) => ({ src: p.src, title: p.name, ratio: p.ratio, media_type: p.media_type, cover_url: p.cover_url, videoRatio: p.video_ratio }));
